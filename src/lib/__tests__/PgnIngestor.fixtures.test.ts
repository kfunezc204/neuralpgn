...
describe.skipIf(!existsSync(WOODPECKER))(
  ...
  () => {
    it('ingests 1131 exercises into 8 stm chapters with one line per DFS leaf and per-exercise intros', { timeout: 30_000 }, () => {
      const text = readFileSync(WOODPECKER, 'utf8')
      const result = new PgnIngestor().ingest(text)

      expect(result.chapters).toHaveLength(8)
      expect(result.chapters.map((c) => c.name)).toContain('4) Easy Exercises')
      expect(result.chapters.map((c) => c.name)).toContain('8) Advanced Exercises')
      ...
      const easy = result.chapters.find((c) => c.name === '4) Easy Exercises')!
      ...
      expect(result.warnings.every((w) => w.code === 'z0_no_variation')).toBe(true)
...
describe.skipIf(!existsSync(SCOTCH))(
  ...
  () => {
    it('ingests the repertoire into 1 chapter using the resolver-chosen side, expanding all 109 Z0 tokens', { timeout: 30_000 }, () => {
